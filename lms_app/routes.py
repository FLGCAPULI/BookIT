...
def delete_book(book_id):
    ...
    try:
        ...
        Transaction.query.filter_by(book_id=book.id).delete()

        Feedback.query.filter_by(book_id=book.id).delete()

        db.session.delete(book)
        db.session.commit()
        flash('Book and associated transactions and feedbacks deleted successfully', 'success')
    except Exception as e:
        db.session.rollback()
        flash('Error deleting book: {}'.format(str(e)), 'danger')
